    ...
    @foreach($teachers as $teacher)
        <div>
            <img style="margin:2px; cursor:pointer"
                 title="{{$teacher->snp}}&#013{{$teacher->position}}{{$teacher->sex=='ж'?($teacher->position=='вчитель' || $teacher->position=='секретар')?'ка':'':''}}@foreach($teacher->subjects as $subject)&#013{{$subject->name}}@endforeach
                         "
