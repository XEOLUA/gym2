<h2 style="text-align: center"><a href="{{route('statisticsman')}}">Статистика МАН</a> <b>{{Request::route()->parameter('pupil')}}</b></h2>
                    ...
                    <div class="stat-block">
                        ...
                        @foreach($dt as $item)
                            <div class="stat-block">
                                ...
                                ступеня, учень: {{$item->pupil}}, керівник: <a href="{{url('statistics/mans/teachers/'.$item->teacher_id)}}">{{$teachers[$item->teacher_id]['snp']}}</a>
                                </span>
                            </div>
                        @endforeach
                    </div>
