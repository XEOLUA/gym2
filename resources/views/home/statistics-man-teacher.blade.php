<h2 style="text-align: center"><a href="{{route('statisticsman')}}">Статистика МАН</a> <b><a href="{{url('teachers/page/'.Request::route()->parameter('teacher'))}}">{{$teachers[Request::route()->parameter('teacher')]->snp}}</a></b></h2>
                    ...
                    <div class="stat-block">
                        ...
                        @foreach($dt as $item)
                            <div class="stat-block">
                                ...
                                ступеня, учень: <a href="{{url('statistics/mans/pupils/'.$item->pupil)}}">{{$item->pupil}}</a>, керівник: {{$teachers[$item->teacher_id]['snp']}}
                                </span>
                            </div>
                        @endforeach
                    </div>
